...
export function academicCreate(name, hobby) {
  ...
  let idNumber = 1; 
  while (dataStore.academics[idNumber] != null) {
    idNumber++;
  }
...
export function courseCreate(academicId, name, description) {
  let courseCode = 1500; 
  while (dataStore.courses[courseCode] != null) {
    idNumber += 10;
  }

  dataStore.courses[courseCode].id = idNumber;
  dataStore.courses[courseCode].name = name;
  dataStore.courses[courseCode].description = description;
