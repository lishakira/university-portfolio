// Exercise: Add numbers in an array together, carrying numbers where
// necessary
// advanced_addition.c
//
...
int sum(int num_lines, int num_digits, int array[MAX_SIZE][MAX_SIZE]) {
    ...
    int col;
    for (col = num_digits; col >= 0; col--) {
        int row;
        for (row = 0; row < num_lines; row++) {
            array[num_lines][col] += array[row][col];
        }
        
        array[num_lines][col] += carry;
        temp = array[num_lines][col];
        
        if (array[num_lines][col] >= 10) {
            array[num_lines][col] %= 10;
        }
        
        carry = (temp - array[num_lines][col]) / 10;
    }
